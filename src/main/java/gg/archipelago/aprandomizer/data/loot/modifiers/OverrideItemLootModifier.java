...
public class OverrideItemLootModifier extends LootModifier {
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        ...
        context.getResolver().lookupOrThrow(Registries.LOOT_TABLE).get(table).ifPresent(
                newTable -> newTable.value().getRandomItemsRaw(
                        context,
                        LootTable.createStackSplitter(context.getLevel(), newLoot::add)
                )
        );
        return newLoot;
    }
